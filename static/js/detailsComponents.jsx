...
const DetailsAddHikeToNewOrExistingList = React.forwardRef((props, ref) => {
  ...
  const addHikeExistingBookmarksList = () => {
...
const HikeDetailsCheckInContainer = () => {
  const session_login = document.querySelector("#login").innerText;
  const hike_id = document.querySelector("#hike_id").innerText;
  ...
  const getCheckIns = () => {
    if (checkInsHeader === "Check Ins For This Hike") {
      getHikeCheckIns();
    } else if (checkInsHeader === "Your Check Ins For All Hikes") {
      getUserCheckIns();
    }
  };

  const getHikeCheckIns = () => {
  ...
  const getUserCheckIns = () => {
    fetch(`/user_check_ins.json`)
      ...
      .then((data) => {
        setCheckIns(data.checkIns);
        setCheckInsHeader("Your Check Ins For All Hikes");
      });
  };
...
const HikeDetailsCommentContainer = () => {
  ...
  const getComments = () => {
    if (commentsHeader === "Comments For This Hike") {
      getHikeComments();
    } else if (commentsHeader === "Your Comments for All Hikes") {
      getUserComments();
    }
  };

  const getHikeComments = () => {
